...
use crate::consts::{DPI, INCHES_PER_METER, EARTH_RADIUS_METERS};
...
pub fn web_mercator_proj(lon: f32, lat: f32, proj_scale_factor: f32) -> (f32, f32) {
...
mod tests {
    ...
    fn assert_eq_approx(a: f32, b: f32, tolerance: f32) {
    ...
    fn mercator_projection() {
        ...
        let (x, y) = super::web_mercator_proj(deg_to_rad(10.), deg_to_rad(10.), scale_factor);
        assert_eq_approx(scale_to_1_dpi(x), 0.17453293, 0.0000001);
        assert_eq_approx(scale_to_1_dpi(y), 0.17542583, 0.0000001);

        let (x, y) = super::web_mercator_proj(deg_to_rad(10.), deg_to_rad(80.), scale_factor);
        assert_eq_approx(scale_to_1_dpi(x), 0.17453293, 0.000001);
        assert_eq_approx(scale_to_1_dpi(y), 2.43624605, 0.000001);

        let (x, y) = super::web_mercator_proj(deg_to_rad(180.), deg_to_rad(-80.), scale_factor);
